...
> Status: functional research preview (`0.7.0`). The core extraction algorithm,
...
* **SDO/AIA Level-1 FITS**: detected full-disk AIA Level-1 maps are registered
  ...
  Version 0.7 also keeps the configured number of prepared Map objects in RAM,
...
4. Optionally select the magnifier in the image toolbar and drag a rectangle to
   ...
   Drag a completed
   control point or label to edit. Full instructions are under Help → Slit / Region Drawing Help.
5. Set scientific slit width, integration statistic, interpolation, tracking,
   and coordinate mode. Display line width is deliberately separate from slit
   width.
...
* Plot overlays each region's NaN-aware mean or sum versus real observation time.
  ...
  display can be switched between bars and step lines.
...
* Region figures expose editable English title/axis labels, time format, line
  style/width, marker, linear/log scale, grid and font sizes. Plot curves retain
