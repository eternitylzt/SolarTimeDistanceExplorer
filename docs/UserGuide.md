...
5. If needed, click the image-toolbar Zoom button and drag around the subfield;
   ...
   Drag any marker after completion to correct it; Esc cancels. Use Help → Slit / Region Drawing
   Help for the same instructions without permanently occupying the control panel.
6. In Scientific slit set Width = 5, Width unit = pixel, Integration = mean,
   Interpolation = linear. The dashed bounds are the actual numerical strip;
   the solid cyan line thickness is only a display style.
...
9. In Time–Distance, leave True observational time on for irregular cadence.
   ...
   the status bar. Clear slope removes all measurement markers. Time labels can
    ...
    size apply both to new measurements and existing visible markers.
...
7. Choose mean or sum, then Plot Time Trend. All regions are overlaid against
   real observation time when available.
...
10. The Region Analysis page can edit its English title and axis labels, time
    tick format, line style/width, marker, linear/log scales, grid and font sizes,
