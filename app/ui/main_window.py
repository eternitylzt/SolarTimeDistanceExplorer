...
from app.ui.dialogs import AnimationExportDialog, ManualTimeDialog, TimeAxisDialog, show_error
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        slope_style.addWidget(QLabel("速度标注字号")); slope_style.addWidget(self.slope_fontsize)
        ...
        for label, control in (("坐标标题字号", self.region_axis_label_size), ("刻度字号", self.region_tick_size)):
        ...
        for control in (self.region_line_width, self.region_axis_label_size, self.region_tick_size):
    ...
    def show_drawing_help(self) -> None:
        QMessageBox.information(
            ...
            "【切片】点击“新建切片”右侧箭头并直接选择直线、折线或平滑曲线，形状立即作用于本次新建。直线依次单击起点和终点后自动完成；折线/平滑曲线逐点单击，在原地双击或单击右键完成。完成后可拖动控制点和标签。\n\n"
            "【区域】点击“绘制新区域”并直接选择形状。圆形：单击圆心，移动鼠标预览，再单击确定半径。长方形：先单击一条边的两个端点，再移动并单击确定高度。多边形：逐点单击，在原地双击或单击右键闭合。\n\n"
            ...
            "科学切片宽度会实际参与法向取样；显示线宽只改变屏幕上的线条粗细，两者完全独立。",
        )
    ...
    def _active_region_changed(self, row: int) -> None:
        if not 0 <= row < len(self.regions):
            ...
            self.path_editor.set_geometry(None); self.region_editor.set_geometry(geometry)
    ...
    def calculate_region_trends(self) -> None:
        ...
        if self.dataset is None:
            return
        completed = [item for item in self.regions if item.complete]
        if not completed:
            self.statusBar().showMessage("请先完成至少一个闭合区域。")
            return
    ...
    def _active_path_changed(self, row: int) -> None:
        ...
        self.active_path_id = geometry.id
        self.path_editor.set_geometry(geometry)
