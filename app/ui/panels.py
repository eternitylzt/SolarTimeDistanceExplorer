...
class PathPanel(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self.new.setMinimumHeight(34)
        self.new.setStyleSheet("QToolButton {background:#176b91; color:white; font-weight:600; padding:5px 12px; border-radius:4px;} QToolButton:hover {background:#2186ac;}")
        ...
        self.generate_button.setStyleSheet("QPushButton {background:#176b91; color:white; font-weight:600; padding:5px; border-radius:4px;}")
...
class RegionPanel(QWidget):
    ...
    calculate_histogram_range = Signal()
    use_td_time_range = Signal()
    export_data = Signal()
    changed = Signal()
    help_clicked = Signal()
    visibility_changed = Signal(int, bool)
    ...
    def __init__(self) -> None:
        ...
        self.histogram_start = QSpinBox(); self.histogram_start.setMinimum(1)
        self.histogram_end = QSpinBox(); self.histogram_end.setMinimum(1)
        self.histogram_step = QSpinBox(); self.histogram_step.setMinimum(1)
        self.histogram_fps = QDoubleSpinBox(); self.histogram_fps.setRange(0.1, 60.0)
        self.histogram_fps.setValue(8.0); self.histogram_fps.setSuffix(" fps")
        trend = QPushButton("绘制已勾选区域的时间变化")
        histogram = QPushButton("绘制当前帧直方图")
        histogram_range = QPushButton("绘制范围直方图序列")
        use_td_range = QPushButton("使用 TD 当前时间范围")
        ...
        histogram_range.clicked.connect(self.calculate_histogram_range)
        use_td_range.clicked.connect(self.use_td_time_range)
        export.clicked.connect(self.export_data)
        trend_row = QHBoxLayout()
        trend_row.addWidget(QLabel("统计量")); trend_row.addWidget(self.statistic)
        trend_row.addWidget(QLabel("形式")); trend_row.addWidget(self.trend_plot_type)
        aform.addRow(trend_row)
        aform.addRow(trend)
        aform.addRow(QLabel("Histogram"))
        bin_row = QHBoxLayout()
        bin_row.addWidget(QLabel("Bin")); bin_row.addWidget(self.bin_width)
        bin_row.addWidget(QLabel("形式")); bin_row.addWidget(self.histogram_plot_type)
        aform.addRow(bin_row)
        aform.addRow("纵轴", self.histogram_y_unit)
        histogram_range_row = QHBoxLayout()
        histogram_range_row.addWidget(QLabel("Start")); histogram_range_row.addWidget(self.histogram_start)
        histogram_range_row.addWidget(QLabel("End")); histogram_range_row.addWidget(self.histogram_end)
        histogram_range_row.addWidget(QLabel("Step")); histogram_range_row.addWidget(self.histogram_step)
        aform.addRow("帧范围", histogram_range_row)
        playback_row = QHBoxLayout()
        playback_row.addWidget(use_td_range); playback_row.addWidget(self.histogram_fps)
        aform.addRow(playback_row)
        histogram_actions = QHBoxLayout()
        histogram_actions.addWidget(histogram)
        histogram_actions.addWidget(histogram_range)
        aform.addRow(histogram_actions)
        for button in (trend, histogram, histogram_range):
            button.setMinimumHeight(32)
            button.setStyleSheet("QPushButton {background:#176b91; color:white; font-weight:600; padding:5px; border-radius:4px;}")
        layout.addWidget(analysis)
        layout.addWidget(export)
        layout.addStretch(1)

    def set_frame_range(self, n_frames: int) -> None:
        """Update inclusive one-based histogram range controls for a dataset."""
        maximum = max(1, int(n_frames))
        for control in (self.histogram_start, self.histogram_end, self.histogram_step):
            control.setMaximum(maximum)
        self.histogram_start.setValue(1)
        self.histogram_end.setValue(maximum)
        self.histogram_step.setValue(1)
