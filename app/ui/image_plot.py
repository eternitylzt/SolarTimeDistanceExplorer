    ...
    def show_frame(
        ...
        if solar_map is not None:
            try:
                ...
                pass
        self.axes.set_title(title)
        self._colorbar = self.figure.colorbar(self._image_artist, ax=self.axes, pad=0.02)
        self._colorbar.set_label("Intensity")
        self._path_artists = []
        ...
        if geometry.show_width_boundaries:
            try:
                ...
                self._path_artists.extend(
                    self.axes.plot(left[:, 0], left[:, 1], "--", color=geometry.display_color, alpha=0.65)
                )
                self._path_artists.extend(
                    self.axes.plot(right[:, 0], right[:, 1], "--", color=geometry.display_color, alpha=0.65)
                )
